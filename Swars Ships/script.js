...
function populateNav(starshipsArray) {
    starshipsArray.forEach(starship => {
        ...
        shipAnchor.addEventListener('click', event => {
            ...
            const foundShip = starshipsArray.find(ship => ship.name === shipName )
            populateView(foundShip)
        })
...
function populateView(ship) {
    removeChildren(view)
    let imgNum = getLastNumber(ship.url)
    let shipImg = document.createElement('img')
    shipImg.src = `https://starwars-visualguide.com/assets/img/starships/${imgNum}.jpg`

    shipImg.addEventListener('error', event => { 
        shipImg.hidden = true
        modalDialog.classList.toggle("is-active")
        modalMessage.textContent = `404 ${ship.name} not found. :(`
       })

    view.appendChild(shipImg)
}

populateNav(starshipsArray)